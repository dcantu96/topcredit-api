module Payments
  def self.get_next_payment_date(installation_date, term_duration_type, i)
    installation_date = installation_date.to_date unless installation_date.is_a?(Date)
  ...
  def self.calculate_two_weeks_payment(installation_date, i)
    ...
    base_date = if installation_date.day <= 15
    ...
    i.times do
      if base_date.day == 15
        ...
        base_date = base_date.beginning_of_month.advance(months: 1).change(day: 15)
      end
    end
  ...
  def self.calculate_payments_count(installation_date, duration_type, total_payments)
    installation_date = installation_date.to_date unless installation_date.is_a?(Date)
    current_date = Date.today
    
    first_payment_date = get_next_payment_date(installation_date, duration_type, 0)
    payment_count = 0
    ...
    while next_payment_date <= current_date && payment_count < total_payments
      payment_count += 1
      next_payment_date = get_next_payment_date(installation_date, duration_type, payment_count)
    end
