class Company < ApplicationRecord
  ...
  def validate_enabled_terms
    ...
    required_duration_type = employee_salary_frequency == "biweekly" ? "two-weeks" : "months"
    required_duration_type_label = employee_salary_frequency == "biweekly" ? "quincenal" : "mensual"
  
    # Fetch only enabled term offerings
    enabled_terms = terms.joins(:term_offerings).where(term_offerings: {enabled: true})
  
    # Validate that all enabled term offerings have the correct duration type
    if enabled_terms.exists? && enabled_terms.where.not(duration_type: required_duration_type).exists?
